...
const dishes = [
  {
    ...
    category: "burger",
    image:
      "https://i.pinimg.com/736x/e8/c2/12/e8c2127a5730742aee1e227f1ee0b784.jpg",
  },
  {
    id: 2,
    ...
    category: "local",
    image:
      "https://i.pinimg.com/736x/25/48/67/2548670de1705009c8e1f5b94a82f0b8.jpg",
  },
  {
    id: 3,
...
class KMCRestaurant {
  ...
  setActiveNav() {
    ...
    links.forEach((link) => {
      // Get the href attribute and convert to pathname
        const href = new URL(link.href, window.location.origin).pathname;
        console.log("Current path:", currentPath, "Link href:", href);

      // If current path matches the link's href, mark as active
      if (currentPath === href) {
  ...
  updateCartCount() {
    document.getElementById("cartCount").textContent = this.cart.reduce(
  ...
  renderCartItems() {
    ...
    document.getElementById(
      "cartTotal"
    ).textContent = `${total.toLocaleString()} XAF`;
  ...
  showCartNotification(itemName) {
    ...
    notification.className =
      "fixed bottom-4 left-4 bg-green-600 text-white p-4 rounded-lg shadow-lg animate-fade-in-down";
  ...
  renderDishes() {
    ...
    container.innerHTML = dishes
      .map(
        (dish) => `
            <div class="group bg-red-100 rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all">
                <img src="${dish.image}" 
                     alt="${dish.title}" 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navLinks.forEach(link => {
        ...
        else if (currentPath.includes('menu') && href.includes('menu')) {
            link.classList.add('active');
        }
